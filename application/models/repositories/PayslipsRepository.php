           ...
           // dd($slip); 
            ...
            {  
                    ...
                    foreach ($payslip['items'] as $key=>$item)  
                    {
                        ...
                        $objPHPExcel->getActiveSheet()->SetCellValue('W' . $row, $item->getEmployee()->getRegularHolidayPay($from, $to));
                        $objPHPExcel->getActiveSheet()->SetCellValue('X' . $row, $item->getEmployee()->getSpecialHolidayPay($from, $to));
                        $objPHPExcel->getActiveSheet()->SetCellValue('Y' . $row, $item->getEmployee()->getGross($from, $to));
                        // $objPHPExcel->getActiveSheet()->SetCellValue('T' . $row, $item->getEmployee()->get#CASHADVANCE;
                        $objPHPExcel->getActiveSheet()->SetCellValue('Z' . $row, $item->getEmployee()->getAbsentDeduction($from, $to, true));
                        $objPHPExcel->getActiveSheet()->SetCellValue('AA' . $row, $item->getEmployee()->getUnderTimeAndLateDeduction($from, $to, 'minute', true));
                        ...
                        $objPHPExcel->getActiveSheet()->SetCellValue('AB' . $row, $item->getEmployee()->getTotalDeductions($from, $to));
                        $objPHPExcel->getActiveSheet()->SetCellValue('AH' . $row, number_format($item->sss, 2));
                        $objPHPExcel->getActiveSheet()->SetCellValue('AI' . $row, number_format($item->sss_employer,2));

                        $objPHPExcel->getActiveSheet()->SetCellValue('AJ' . $row, $item->getEmployee()->getGeneratedSSSEC($from, $to));
                        ...
                        $objPHPExcel->getActiveSheet()->SetCellValue('AK' . $row, ((float)$item->getEmployee()->getGeneratedSSSEmployee($from, $to) + (float)$item->getEmployee()->getGeneratedSSSEmployer($from, $to)));

                        $objPHPExcel->getActiveSheet()->SetCellValue('AL' . $row, $item->getEmployee()->getGeneratedPagibig($from, $to));
                        $objPHPExcel->getActiveSheet()->SetCellValue('AM' . $row, $item->getEmployee()->getGeneratedPagibig($from, $to));
                        $objPHPExcel->getActiveSheet()->SetCellValue('AN' . $row, ((float)$item->getEmployee()->getGeneratedPagibig($from, $to)) * 2);
                        $objPHPExcel->getActiveSheet()->SetCellValue('AO' . $row, $item->getEmployee()->getGeneratedPhilhealth($from, $to));
                        $objPHPExcel->getActiveSheet()->SetCellValue('AP' . $row, $item->getEmployee()->getGeneratedPhilhealth($from, $to));
                        $objPHPExcel->getActiveSheet()->SetCellValue('AQ' . $row, ((float)$item->getEmployee()->getGeneratedPhilhealth($from, $to)) * 2);
                        // $objPHPExcel->getActiveSheet()->SetCellValue('AL' . $row, $item->getEmployee()->getGeneratedPhilhealth($from, $to));
                        $objPHPExcel->getActiveSheet()->SetCellValue('AR' . $row, $item->getEmployee()->getNet($from, $to));
